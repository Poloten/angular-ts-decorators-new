...
function NgModule(_a) {
    ...
    return function (Class) {
        ...
        if (runMethods) {
            runMethods.forEach(function (r) { module$1.run(r); });
        }
        var configMethods = getMetadata(metadataKeys.config, Class.prototype);
        if (configMethods) {
            configMethods.forEach(function (c) { module$1.config(c); });
        }
...
export { Component, Config, Declaration, Directive, ElementRef, HostListener, Inject, Injectable, Input, NgModule, Output, Pipe, Run, Type, ViewChild, ViewChildren, ViewParent, addStaticInjections, camelToKebab, defineMetadata, getAttributeName, getMetadata, getTypeDeclaration, getTypeName, injectionsKey, isAttributeSelector, isType, kebabToCamel, metadataKeys, ngLifecycleHooksMap, platformBrowserDynamic };
